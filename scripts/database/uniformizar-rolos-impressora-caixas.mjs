...
const SKU_ROLOS = "CONS-OPS-PRINTER-ROLLS";
const NOVA_UNIDADE = "caixa";
const UPDATED_BY = "OPS-UNIFORMIZATION-ROLOS-IMPRESSORA";

async function main() {
  console.log("=== RIBBAI - Uniformização Rolos Impressora ===");
  console.log(`SKU: ${SKU_ROLOS}`);
  ...
  try {
    ...
    if (!produto) {
      console.log(`❌ Produto não encontrado: ${SKU_ROLOS}`);
      return;
    }
...
main()
  .then(() => {
    console.log("✅ Uniformização dos Rolos Impressora concluída!");
  })
  .catch((error) => {
