...
const TRANSACTION_AT = new Date("2026-06-18T12:00:00.000Z");
...
async function generateInventoryChangeSummary(outputDir, transactions) {
  ...
  const content = [
    ...
    "## Notas",
    "",
    "Entrada de stock registada para reforco de consumiveis essenciais apos contagem fisica de 16-06-2026. Historico anterior mantido sem alteracoes.",
    "",
  ].join("\n");
...
async function generateAlertSummary(outputDir) {
  ...
  const content = [
    ...
    "## Nota",
    "",
    "Alertas calculados com base em `InventoryItem.reorderPoint` apos aplicar a entrada de stock desta referencia.",
    "",
  ].join("\n");

  const filePath = path.join(outputDir, `${DATE}-alert-summary.md`);
  await writeFile(filePath, content, "utf8");
  return { filePath, items, critical };
}
...
async function generateDashboardMetrics(outputDir, items) {
  ...
  const content = [
    ...
    toMarkdownTable(
      ...
      categoryRows.map((row) => [row.category, String(row.items), String(row.Saudavel), String(row.Critico)])
    ),
    "",
    "## Nota de gestao",
    "",
    "Atualizacao de dashboard apos entrada de stock de consumiveis em 18-06-2026. Verificar desaparicao de alertas de rutura no SKU CLEAN-ALCOHOL.",
    "",
  ].join("\n");
...
async function main() {
  ...
  try {
    await runCommand(
      "npx",
      ["tsx", "scripts/database/record-stock-in-2026-06-18.ts", ...(force ? ["--force"] : [])],
      { cwd: rootDir }
    );
    ...
    const alcohol = alert.items.find((item) => item.sku === "CLEAN-ALCOHOL");
    console.warn(
      `Stock-in artifacts generated in: ${outputDir}. CLEAN-ALCOHOL=${formatQuantity(
        alcohol?.currentStock ?? 0
      )} ${alcohol?.unit ?? "unidade"}`
    );
  } finally {
