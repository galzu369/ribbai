...
const dataPath = path.join(rootDir, "pdf", "data", "opening-checklist.json");
const outputDir = path.join(rootDir, "docs", "operational-records", "checklists");
const outputPath = path.join(outputDir, "opening-checklist-ribbai-fillable.pdf");
...
function drawFooter(targetPage, number, total) {
  ...
  targetPage.drawText("RIBBAI OPS · Checklist de Abertura", {
    x: margin,
    y: 22,
    size: 8,
    font: regularFont,
...
function drawSection(section) {
  const taskHeights = section.tasks.map((task) => {
    ...
    return Math.max(36, lines.length * 15 + 16);
  });
  ...
  section.tasks.forEach((task, index) => {
    ...
    addCheckBox(task.id, margin + 12, y - 25);
    drawWrappedText(task.label, margin + 40, y - 21, contentWidth - 56, 11.5, 15);
    y -= rowHeight;
  });
...
function drawAccountability() {
  ...
  drawText("Abertura e responsabilidade", margin + 14, y - 26, {
    size: 17,
    bold: true,
  });
  ...
  const topY = y - 58;
  const startField = checklist.accountability.timeFields[0];
  const endField = checklist.accountability.timeFields[1];
  ...
  addTextField(startField.id, "Inicio", leftX + 10, topY - 24, boxWidth - 20, {
    labelWidth: 42,
  });
  addTextField(endField.id, "Fim", leftX + 10, topY - 50, boxWidth - 20, {
    labelWidth: 42,
  });
  ...
  drawText(checklist.accountability.responsibleStaffLabel, rightX + 10, topY - 17, {
    size: 8,
    bold: true,
    color: colors.muted,
  });
  ...
  drawText(checklist.accountability.observationsLabel, leftX + 10, obsY - 15, {
    size: 8,
    bold: true,
    color: colors.muted,
  });
